...
# Это относится к колонкам... Нужно думать про структуру и связи функции между собой. А также про организацию ссылок (чтобы можно было перейти в соответствующую книжку).
#
#

# Возможность организации условных вычислений
#
# Иногда нужны вычислить выражение, которое содержит условия "если то". Для этого есть функции
#
# * when() - позитивная часть "если то"
# * otherwise() - негативная часть "если то"
...
sp = SparkSession.builder.master("local").appName("when").enableHiveSupport().getOrCreate()
...
# **smoke** добавим колонку - наличие граней

geoDf.when(geoDf.points>1,1).show()
